...
export default function SettingsForm({ profile, userId }: { profile: any; userId: string }) {
  ...
  return (
    <form action={handleSubmit} className="space-y-5">
      ...
      <div className={`bg-white rounded-xl border border-gray-200 p-6 space-y-4 ${!isPro ? 'opacity-60' : ''}`}>
        <div className="flex items-start justify-between">
          <div>
            <h2 className="font-semibold text-gray-900">Reminder email template</h2>
            <p className="text-xs text-gray-400 mt-0.5">Sent automatically to clients when invoices are overdue.</p>
          </div>
          {!isPro && (
            <span className="text-xs font-medium bg-blue-100 text-blue-700 px-2.5 py-1 rounded-full whitespace-nowrap">Pro only</span>
          )}
        </div>
        {!isPro ? (
          <p className="text-sm text-gray-500">Upgrade to Pro to customise your reminder email template.</p>
        ) : (
          <>
