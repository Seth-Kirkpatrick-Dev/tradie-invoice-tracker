...
export default function NewInvoiceForm({ clients, defaultCurrency, defaultTaxRate, defaultTaxLabel, defaultPaymentMethod }: Props) {
  ...
  function addLine() { setLineItems(prev => [...prev, { description: '', quantity: 1, unit_price: 0 }]) }
  ...
  async function handleSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      ...
      if (result?.error) setError(typeof result.error === 'string' ? result.message ?? result.error : '')
    })
  }
    ...
    <form action={handleSubmit} className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
        ...
        <div>
          ...
          <select name="client_id" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div>
          ...
          <input name="description" type="text" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Bathroom renovation — labour and materials" />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Issue date</label>
            <input name="issue_date" type="date" defaultValue={today} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Due date</label>
            <input name="due_date" type="date" defaultValue={thirtyDays} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <div className="mt-5 pt-4 border-t border-gray-100 space-y-1.5">
          ...
          <div className="flex items-center justify-between text-sm text-gray-600">
            <div className="flex items-center gap-2">
              <input name="tax_label" type="text" defaultValue={defaultTaxLabel} className="w-12 text-xs border-0 border-b border-gray-300 focus:outline-none focus:border-blue-500 bg-transparent" />
              <input name="tax_rate" type="number" min="0" max="100" step="0.1" value={taxRate} onChange={e => setTaxRate(parseFloat(e.target.value) || 0)} className="w-14 text-xs border-0 border-b border-gray-300 focus:outline-none focus:border-blue-500 bg-transparent text-right" />
              <span className="text-xs">%</span>
            </div>
            <span>{fmt(taxAmount)}</span>
          </div>
          <div className="flex justify-between font-bold text-gray-900 pt-1">
            <span>Total</span><span>{fmt(total)} <span className="font-normal text-gray-500 text-xs">{defaultCurrency}</span></span>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
        ...
        <div>
          ...
          <textarea name="payment_method" rows={3} defaultValue={defaultPaymentMethod} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" placeholder="Bank account, reference, etc." />
        </div>
        <div>
